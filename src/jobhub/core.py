...
def normalize_job(raw,source):
    ...
    return {'id':jid,'title':title,'company':company,'url':url,'location':str(raw.get('location') or '').strip(),'description':re.sub(r'<[^>]+>',' ',str(raw.get('description') or '')).strip(),'source':source,'published_at':raw.get('published_at') or raw.get('date') or '', 'work_mode':raw.get('work_mode') or ('remote' if raw.get('remote') else ''),'source_kind':raw.get('source_kind') or 'unknown','source_url':raw.get('source_url') or ''}
